...
Maven uses a plugin called "surefire" to run unit tests. The default configuration of this plugin compiles and runs all classes in `src/tets/java` with a name matching `*Test`. You can run the tests on the command line like this
